...
namespace DialogueTweak.Interfaces
{
    public class GUIChat
    {
        ...
        public static void GUIDrawInner() {
            ...
            Main.spriteBatch.Draw(Main.chatBackTexture, new Vector2(Main.screenWidth / 2 - Main.chatBackTexture.Width / 2, 100f), new Rectangle(0, 0, Main.chatBackTexture.Width, (int)(linePositioning >= 15 ? 450 : 2 + linePositioning * 30)), panelColor, 0f, default(Vector2), 1f, SpriteEffects.None, 0f);
            if (linePositioning >= 15) Main.spriteBatch.Draw(Main.chatBackTexture, new Vector2(Main.screenWidth / 2 - Main.chatBackTexture.Width / 2, 550f), new Rectangle(0, 30, Main.chatBackTexture.Width, (int)(2 + (linePositioning - 15) * 30)), panelColor, 0f, default(Vector2), 1f, SpriteEffects.None, 0f);
            Main.spriteBatch.Draw(Main.chatBackTexture, new Vector2(Main.screenWidth / 2 - Main.chatBackTexture.Width / 2, 102 + linePositioning * 30), new Rectangle(0, Main.chatBackTexture.Height - 30, Main.chatBackTexture.Width, 30), panelColor, 0f, default(Vector2), 1f, SpriteEffects.None, 0f);
            // 碰撞箱
            var rectangle = new Rectangle(Main.screenWidth / 2 - Main.chatBackTexture.Width / 2, 100, Main.chatBackTexture.Width, (int)(2 + linePositioning * 30 + 30));

            // 文字，还有一个方框深黑底
            if (amountOfLines > 1) {
                Main.spriteBatch.Draw(ChatStringBack, new Vector2(268 + (Main.screenWidth - 800) / 2, 145f), new Rectangle(0, 0, ChatStringBack.Width, (amountOfLines - 1) * 30), panelColor, 0f, default(Vector2), 1f, SpriteEffects.None, 0f);
                Main.spriteBatch.Draw(ChatStringBack, new Vector2(268 + (Main.screenWidth - 800) / 2, 145f + (amountOfLines - 1) * 30), new Rectangle(0, ChatStringBack.Height - 30, ChatStringBack.Width, 30), panelColor, 0f, default(Vector2), 1f, SpriteEffects.None, 0f);
            }
            // 只有一行，为了避免黑底显示错误，做一下特判
            else {
                Main.spriteBatch.Draw(ChatStringBack, new Vector2(268 + (Main.screenWidth - 800) / 2, 145f), new Rectangle(0, 0, ChatStringBack.Width, 15), panelColor, 0f, default(Vector2), 1f, SpriteEffects.None, 0f);
                Main.spriteBatch.Draw(ChatStringBack, new Vector2(268 + (Main.screenWidth - 800) / 2, 145f + 15), new Rectangle(0, ChatStringBack.Height - 15, ChatStringBack.Width, 15), panelColor, 0f, default(Vector2), 1f, SpriteEffects.None, 0f);
            }
            int letterCount = 0;
            // 对话文本
            for (int i = 0; i < amountOfLines; i++) {
                ...
                if (text != null) {
                    ...
                    foreach (var snippet in snippets) {
                        ...
                        if (letterCount > DialogueTweak.letterAppeared) {
                            // 计算出多了多少个
                            int outOfRange = letterCount - (int)DialogueTweak.letterAppeared;
            ...
            if (Main.npcChatCornerItem != 0) {
                Vector2 position = new Vector2(Main.screenWidth / 2 + Main.chatBackTexture.Width / 2 - 8, 30 + (amountOfLines + 3) * 30 + 30);
            ...
            // 人像背景框，以及名字和文本的分割线
            Main.spriteBatch.Draw(PortraitPanel, new Vector2(Main.screenWidth / 2 - PortraitPanel.Width / 2, 115f), null, Color.White * 0.92f, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
            ...
            if (Main.LocalPlayer.talkNPC >= 0 && Main.LocalPlayer.sign == -1 && Main.npc[Main.LocalPlayer.talkNPC] != null && Main.npc[Main.LocalPlayer.talkNPC].active) {
                ...
                var position = new Vector2(Main.screenWidth / 2 - PortraitPanel.Width / 2, 115f) + new Vector2(62f, 48f);
                ...
                Main.spriteBatch.Draw(value, position, talkNPC.frame, talkNPC.GetAlpha(Color.White), 0f, origin, scale, SpriteEffects.FlipHorizontally, 0f);
                if (talkNPC.color != default(Color))
                    Main.spriteBatch.Draw(value, position, talkNPC.frame, talkNPC.GetColor(talkNPC.color), 0f, origin, scale, SpriteEffects.FlipHorizontally, 0f);

                // 名字，用DeathText因为它大而且清晰
                Utils.DrawBorderStringFourWay(Main.spriteBatch, Main.fontDeathText, talkNPC.GivenOrTypeName, 270f + (Main.screenWidth - 800) / 2, 106, textColor, Color.Black, Vector2.Zero, 0.6f);
            ...
            if (Main.LocalPlayer.sign > -1) {
                ...
                var position = new Vector2(Main.screenWidth / 2 - PortraitPanel.Width / 2, 115f) + new Vector2(62f, 50f);
                // 重新开启Main.spriteBatch，以去掉不明觉厉的UI绘制的一层模糊滤镜
                Main.spriteBatch.End();
                Main.spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, Main.DefaultSamplerState, null, null, null, Main.UIScaleMatrix);

                Main.spriteBatch.Draw(value, position, null, Color.White, 0f, value.Size() / 2f, 2f, SpriteEffects.None, 0f);
                Utils.DrawBorderStringFourWay(Main.spriteBatch, Main.fontDeathText, text, 270f + (Main.screenWidth - 800) / 2, 106, textColor, Color.Black, Vector2.Zero, 0.6f);
            ...
            if (!Main.editSign || !PlayerInput.UsingGamepad) {
                ...
                Main.spriteBatch.Draw(GreyPixel, new Vector2(breakPixel * 2 + Main.screenWidth / 2 - Main.chatBackTexture.Width / 2, linePositioning * 30 + 70), null, Color.White * 0.9f, 0f, Vector2.Zero, new Vector2(Main.chatBackTexture.Width - breakPixel * 4, 3f), SpriteEffects.None, 0f);
            }
