...
internal class PortraitDrawer : ModSystem
{
    ...
    private void DrawNPCPortrait(SpriteBatch sb, Color textColor, Rectangle panel) {
        ...
        if (talkNPC.active) {
            ...
            var preference = Main.DialoguePortraitPreference;

            if (preference == Main.DialoguePortraitDrawOption.Detailed) {
                bool success = DrawNPCPortraitDetailed(sb, talkNPC, previewBox);
                if (!success)
                    DrawNPCInBestiary(sb, talkNPC, previewBox, profile: true);
            }
            else if (preference == Main.DialoguePortraitDrawOption.CloseUp) {
                DrawNPCInBestiary(sb, talkNPC, previewBox, profile: true);
            }
            else if (preference == Main.DialoguePortraitDrawOption.FullBodyRetro) {
                DrawNPCInBestiary(sb, talkNPC, previewBox, profile: false);
            }
            else { // Disabled → mod's Live Reaction
                if (screenTargetUnavailable)
                    DrawNPCInBestiary(sb, talkNPC, previewBox, profile: false);
                else
    ...
    private bool DrawNPCPortraitDetailed(SpriteBatch sb, NPC talkNPC, Rectangle previewBox) {
        if (!NPCID.Sets.NPCPortraits.TryGetValue(talkNPC.type, out var value))
            return false;
        ...
        sb.Draw(texture, previewBox.Center.ToVector2(), null, Color.White, 0f, texture.Size() / 2f, 1f,
    ...
    private void DrawNPCRenderedInWorld(SpriteBatch sb, NPC talkNPC, Rectangle previewBox) {
        ...
        DrawScreenTargetPortrait(sb, talkNPC.Center, npcSize, Main.GameViewMatrix.Zoom.X - 1f, previewBox);
        DrawPortraitPanelFront(previewBox);
    }

    /// <summary>绘制屏幕目标到肖像框，自动处理重力方向和缩放偏移</summary>
    private static void DrawScreenTargetPortrait(SpriteBatch sb, Vector2 worldCenter, int size, float extraZoom,
        Rectangle previewBox) {
        int offset = size / 2;

        // 根据重力方向调整位置
        var effects = SpriteEffects.None;
        var source = new Rectangle((int)worldCenter.X - offset, (int)worldCenter.Y - offset - 6, size, size);
        source.Offset((-Main.screenPosition).ToPoint());
        if (Main.LocalPlayer.gravDir is -1) {
            source.Y = Main.screenHeight - source.Y - size;
        ...
        var screenOffset = (worldCenter - Main.Camera.Center) * extraZoom;
        ...
        // panel front drawn by caller, since DrawNPCInBestiary draws its own
    }
    ...
    private void DrawSignPortrait(SpriteBatch sb, Color textColor, Rectangle panel) {
        ...
        DrawScreenTargetPortrait(sb, signCenter, tileSize, Main.GameViewMatrix.Zoom.X - 1f, previewBox);
    ...
    private void DrawInScissor(SpriteBatch sb, Rectangle boundingBox, Action drawAction) {
        ...
        sb.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, Main.DefaultSamplerState, null, Main.ScissorState,
            null,
            Main.UIScaleMatrix);
    ...
    public static void DoNPCPortraitHop() {
        ...
        if (!Configuration.Instance.PortraitAnimation)
            npcChatPortraitFrameCounter = 120f;
    }
    ...
    internal static void DrawPortrait(SpriteBatch sb, Color textColor, Rectangle panel) {
        ...
        if (npcChatPortraitFrameCounter <= 120f) {
            double max = 80.0;
            float num10 = 0.25f;
            float num11 = (float)Utils.EaseOutBounce(Utils.Clamp((int)npcChatPortraitFrameCounter, 0.0, max) / max);
        ...
        Main.spriteBatch.Draw(ModAsset.PortraitPanel_Overlay.Value, previewBox.Location.ToVector2(), null,
    ...
    private void DrawPortraitPanelFront(Rectangle previewBox) {
        var pos = previewBox.Center.ToVector2();
        var asset = ModAsset.PortraitPanel_Front.Value;
        Main.spriteBatch.Draw(ModAsset.PortraitPanel_Front.Value, pos, null,
            Color.White * 0.92f, 0f, asset.Size() / 2f, 1f, SpriteEffects.None, 0f);
    }
}
